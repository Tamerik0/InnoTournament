...
import dev.necr0manthre.innotournament.tournament.events.ArchitecturyEventBasedTournamentEvent;
...
public class PlayerTickEvent extends ArchitecturyEventBasedTournamentEvent<TickEvent.Player, PlayerHolder> {
    ...
    public TickEvent.Player getListener(Consumer<PlayerHolder> callback) {
        return player -> {
            if (player instanceof ServerPlayer serverPlayer && TeamManager.getPlayersTeam(PlayerManager.getEntity(serverPlayer)).isPresent() && counter.computeIfAbsent(player.getUUID(), u -> 0
            ) == 0) {
                callback.accept(new PlayerHolder(PlayerManager.getEntity(serverPlayer)));
            }
