...
public interface InnoTournamentEventHandlers {
    ...
    IParser<String, ITournamentEventListener<ISourcePlayerProvider>> SCORE_FROM_LIVES = register(new MultiArgumentParser<>("add_score_by_lives",
            scores ->
                    (tournament, playerProvider) ->
                            TeamManager.getPlayersTeam(playerProvider.getSourcePlayer()).ifPresent(
                                    team ->
                                            tournament.addScores(team, Integer.parseInt(scores[TournamentPlayer.getTournamentData(playerProvider.getSourcePlayer()).lives])))));
